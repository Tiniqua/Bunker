...
public class BlueDoor : MonoBehaviour
{
    ...
    {
        if (other.tag == "Player" && (keyCards.GetComponent<DoorControl>().blueKeyCard)) //If the player is within the range of the red door and they have collected the red keycard
        {
            unlockDoorMessage.SetActive(true);
            canOpenDoor = true;
        }
        if (other.tag == "Player" && (!keyCards.GetComponent<DoorControl>().blueKeyCard)) //If the player is within the range of the red door and they have not collected the red keycard
        {
